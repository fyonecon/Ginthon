...
class rand_token:
    ...
    # 生产token
    @staticmethod
    def make(app_class, salt_str, timeout_s, config):
        CONFIG = config
        #
        if len(app_class) == 0:
        ...
        if timeout_s < 10:
            ...
            pass
        #
        new_str = app_class + "#@" + func.md5(salt_str + "_-@!_" + app_class) + "#@" + str(
            func.get_time_ms()) + "#@" + str(timeout_s * 1000) + "#@" + func.rand_range_string(4, 6)
        #
        return func.str_encode(new_str, CONFIG["pywebview"]["secret_key"])

    # 校验token
    @staticmethod
    def check(app_class, salt_str, config, check_str):
        CONFIG = config
        #
        if len(app_class) == 0:
            app_class = CONFIG["app"]["app_class"]
        ...
        try:
            ...
            #
            OK = app_class == _app_class and the_salt_str == _salt_str and (
                        _old_time_ms + _timeout_ms) >= func.get_time_ms()
            if OK:
                return True, "参数已匹配"
            else:
