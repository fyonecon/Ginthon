...
# 获取完整的请求链接
def request_full_href(request):
    return request.url

# flask万能api取值方法
def request_input(request, key: str = ""):
    """
    统一获取请求参数（支持GET/POST）

    Args:
        request: Flask request对象
        key: 参数名，空字符串则返回整个参数字典
    ...
    if not key:
        # 如果没有指定key，返回整个参数字典
        return get_all_request_params(request)
...
# 获取所有请求参数
def get_all_request_params(request):
    """获取所有请求参数"""
    params = {}
