<script>
    ...
    let language_index = now_language(); // 更新选中
                        ...
                        <Dialog.Positioner class="fixed inset-0 z-50 flex justify-center items-center p-4">
                            <Dialog.Content class="card bg-surface-100-900 w-full max-w-xl p-4 space-y-4 shadow-xl {animation}">
                                ...
                                <footer class="flex justify-end gap-2 select-none">
                                    ...
                                    <button type="button" class="btn preset-filled-primary-500" on:click={()=>choose_language("en")}>{func.get_translate("btn_save")}</button>
                                </footer>
                            </Dialog.Content>
                        </Dialog.Positioner>
                    </Portal>
                        ...
                        <Dialog.Positioner class="fixed inset-0 z-50 flex justify-center items-center p-4">
                            <Dialog.Content class="card bg-surface-100-900 w-full max-w-xl p-4 space-y-4 shadow-xl {animation}">
                                ...
                                <footer class="flex justify-end gap-2 select-none">
                                    ...
                                    <button type="button" class="btn btn- preset-filled-primary-500" on:click={()=>choose_language("zh")}>{func.get_translate("btn_save")}</button>
                                </footer>
                            </Dialog.Content>
                        </Dialog.Positioner>
                    </Portal>
