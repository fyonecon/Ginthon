...
def kill_pid_process(pid, timeout=6):
    try:
        ...
        print(f"准备终止进程: {process.name()} (PID: {pid})")
        print(f"进程状态: {process.status()}", process.is_running())
...
def watch_pid():
    ...
    def do_pid_killer():
        ...
        if process3.status() != "running" or process3.status() == "zombie":
            print("必要进程没在运行。。。自动杀死所有Ginthon进程。。。", process3_pid)
            # pid
            ...
            print("必要进程正常运行。。。", process3_pid, process3.is_running(), process3.status())
        pass
...
def watch_processes(_process1_pid, _process2_pid, _process3_pid):
    ...
    print("🚩检测进程：watch_processes：", [process1_pid, process2_pid, process3_pid])
    #
